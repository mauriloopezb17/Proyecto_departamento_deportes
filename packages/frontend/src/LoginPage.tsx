...
function LoginPage() {
  ...
  const handleVerifyCode = async (e: FormEvent) => {
    ...
    setResetLoading(true)
    try {
      const data = await apiFetch<{ valid: boolean; reset_token: string }>(
        '/api/auth/verify-reset-code',
        { method: 'POST', body: JSON.stringify({ email: resetEmail.trim(), codigo: code }) }
      )
  ...
  const handleResetPassword = async (e: FormEvent) => {
    ...
    try {
      await apiFetch('/api/auth/reset-password', {
        method: 'POST',
        body: JSON.stringify({ reset_token: resetToken, nueva_password: newPassword }),
      })
      setResetStep('done')
    } catch (err: any) {
          ...
          {view === 'login' && (
            <>
              ...
              <form className="login-form" noValidate onSubmit={handleLogin}>
                ...
                <button type="button" className="google-btn" onClick={() => (window.location.href = `${API_BASE}/api/auth/google`)}>
